...
**Arcade Shields will work only with micro:bit V2**; they will not work with previous versions of the micro:bit.
...
requirements for a MakeCode Arcade shield for micro:bit V2.
...
PDF schematics of our hardware reference designs are available in this folder. For some of these PDFs, if you view with Adobe Reader
...
or [file an issue](https://github.com/microsoft/pxt-arcade-hardware-designs/issues) in this repo to provide comments and feedback.

We will provide the final version of our specification and updated reference designs around February 2021.
Please do not sell or distribute devices before the specifications are finalized, as
**they may stop working with a released version of MakeCode Arcade** and therefore cannot be approved by 
Microsoft and The Micro:bit Educational Foundation.

**NOTICE:
This information is provided “as-is”.
You bear the risk of using it.
Some information relates to pre-released specification which may change without notice.
...
Finally, the user plugs their micro:bit V2 into an Arcde shield and plays the game.
...
To set `HW_CFGx` to a zero or one, it must be pulled low (to `GND`) or high (to `VUBIT`).
...
This passes through a 300mΩ shunt to reduce the voltage to around 3.25V, depending on current draw.
(Conveniently there is already a 300mΩ ferrite on the design to reduce EMI, so we use a second of these as the shunt rather 
...
If the Arcade Shield does not have batteries fitted and the micro:bit is powered from 2xAA batteries (instead of via USB), only 3.0V will be available. This should be
avoided because shield operation will be unreliable; in particular the display backlight may not function correctly.

When a micro:bit is running a game and the Arcade shield screen is at full brightness the power draw should be ~45mA for a 1.77"
...
[JACDAC](https://aka.ms/jacdac) is a plug-and-play protocol and hardware/software stack for connecting microcontrollers and peripherals
that is being actively developed by Microsoft. JACDAC's flexible topology supports dynanic discovery and power delivery, thereby providing
an intuitive and low-cost way for users to extend their physical computing devices and projects with new hardware. 
Microsoft will provide a first-class JACDAC experience in MakeCode via both Arcade and micro:bit editors. This will integrate the auto-detection, programming, simulation and operation of JACDAC devices.

JACDAC uses three wires to support 1Mbps half-duplex serial communications at 3.3V logic levels and power delivery of up to 500mA at 5V. It uses a custom 3-pin PCB edge connector in conjunction with easy-to-use cables. It is a true bus topology that allows multiple devices to be connected directly to the same bus, but unlike I2C all devices have unique IDs.
...
Users of shields that include JACDAC support will be able to use JACDAC modules in the future, as they become available.

A set of specifications for building these JACDAC modules - that will work with Arcade shields and in other scenarios - is planned for early 2021.
JACDAC also supports communications with other MakeCode Arcade devices (that might not support micro:bit radio communications) for multiplayer gaming.
...
can be included. These components allow the Arcade shield to safely and reliably provide "basic" power to the JACDAC bus. 
...
3.84V (absolute minimum) and 5.25V (absolute maximum) when the current can reach a maximum of 100 mA.
In order to ensure at least 3.84V at the JACDAC connector, the input to the current limiter must be at least 4.05V. This
rules out supplying power directly from 3x AA batteries (which will quickly fall below 1.35V each) or from a LiPo battery;
a DC-DC converter must be used for these battery configurations.

In the future, Microsoft will release more details of JACDAC operation including the option of providing in excess of 100mA 
at a slightly higher voltage. **Please do not design alternative JACDAC power circuits because they may not be compatible 
with the forthcoming JACDAC specification.**
...
JACDAC connectors that support power delivery should be tested to ensure that the power delivered to the bus is current limited as expected, 
...
device that supports up to 100mA is 3.84 - 5.25V; remember to allow for voltage measurement tolerance. No components should become hot to the touch
at any point. Microsoft will outline more detailed JACDAC testing requirements at a future date.
